...
function ContentHeader() {
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-6 h-11 border-b border-border bg-card/60">
      <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
        <span>{mod?.label}</span>
        <ChevronRight className="size-3.5" />
        <span style={{ color: "var(--gov-text)", fontWeight: 500 }}>{pg?.label}</span>
      </div>
      <DropdownMenu>
        <DropdownMenuTrigger className="flex items-center gap-1.5 rounded-md border border-border px-2.5 h-8 text-xs hover:bg-accent disabled:opacity-70" disabled={!canSwitchScope}>
    ...
    <div className="h-screen w-screen flex flex-col bg-background overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto gov-scroll p-6">

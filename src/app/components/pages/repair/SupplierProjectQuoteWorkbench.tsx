// 关联业务：施工单位查看维修工程邀价范围，并在线提交与当前方案和邀价轮次绑定的报价原件。
...
export function SupplierProjectQuoteWorkbench() {
  ...
  async function submitQuote() {
    if (!selected || !attachment || selected.invitation.status !== "PENDING") return;
    setSubmitting(true);
    try {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
      <SectionCard title="维修工程邀价" extra={<Button size="icon" variant="ghost" title="刷新工程邀价" onClick={() => void reload()} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}</Button>} bodyClassName="p-0">
        {loading && opportunities.length === 0 ? <div className="flex items-center justify-center py-14 text-sm text-muted-foreground"><Loader2 className="mr-2 size-4 animate-spin" />加载中</div> : opportunities.length === 0 ? <div className="flex flex-col items-center px-5 py-14 text-center"><Inbox className="mb-3 size-8 text-muted-foreground/60" /><div className="text-sm font-medium">当前没有维修工程邀价</div><div className="mt-1 text-xs text-muted-foreground">物业发出项目邀价或报价修订要求后会显示在这里</div></div> : <div className="divide-y">{opportunities.map((opportunity) => <button key={opportunity.invitation.invitationId} type="button" onClick={() => setSelectedId(opportunity.invitation.invitationId)} className={`w-full px-4 py-3 text-left hover:bg-muted/40 ${selectedId === opportunity.invitation.invitationId ? "bg-primary/5" : ""}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-sm font-medium">{opportunity.projectName}</div><div className="mt-1 text-xs text-muted-foreground">{opportunity.projectNo} · 第 {opportunity.invitation.invitationRound} 轮</div></div><StatusChip tone={opportunity.invitation.status === "PENDING" ? "warning" : "success"}>{opportunity.invitation.status === "PENDING" ? opportunity.invitation.invitationType === "REVISION" ? "待修订" : "待报价" : "已提交"}</StatusChip></div><div className="mt-2 text-xs text-muted-foreground">截止 {opportunity.invitation.deadline ? new Date(opportunity.invitation.deadline).toLocaleString("zh-CN", { hour12: false }) : "未设置"}</div></button>)}</div>}
      </SectionCard>

      <SectionCard title={selected?.projectName ?? "工程报价"} desc={selected ? `${selected.projectNo} · 当前方案不展示物业内部估算金额` : undefined} bodyClassName="min-h-[420px]">
        {!selected ? <div className="py-20 text-center text-sm text-muted-foreground">请选择一条工程邀价</div> : (
          <div className="space-y-5">
            ...
            {selected.latestQuote && <div className="flex flex-wrap items-center justify-between gap-3 border-y bg-muted/20 px-3 py-3 text-sm"><div><span className="font-medium">最近报价 ¥{Number(selected.latestQuote.quoteAmount).toLocaleString("zh-CN", { minimumFractionDigits: 2 })}</span><span className="ml-3 text-muted-foreground">第 {selected.latestQuote.revisionNo} 版</span></div><StatusChip tone={selected.latestQuote.quoteStatus === "REVISION_REQUESTED" ? "warning" : "success"}>{selected.latestQuote.quoteStatus === "REVISION_REQUESTED" ? "物业要求修订" : "已在线确认"}</StatusChip></div>}
            {selected.invitation.status === "PENDING" ? <div className="grid gap-4 border-t pt-5 md:grid-cols-2"><div><Label>含税报价总额</Label><Input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} /></div><RepairProjectFileUpload projectId={selected.projectId} label="报价原件" value={attachment} onUploaded={setAttachment} /><div className="md:col-span-2"><Label>报价说明</Label><Textarea value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="填写工期、税费、主要材料及报价边界" /></div><div className="md:col-span-2 flex justify-end"><Button disabled={submitting || Number(amount) <= 0 || !attachment} onClick={() => void submitQuote()}>{submitting ? <Loader2 className="mr-1 size-4 animate-spin" /> : <FileCheck2 className="mr-1 size-4" />}提交并确认报价</Button></div></div> : <div className="py-5 text-center text-sm text-muted-foreground">本轮报价已提交，等待物业比价；如需修订，物业会发出新的修订轮次。</div>}
          </div>
        )}
      </SectionCard>
    </div>
